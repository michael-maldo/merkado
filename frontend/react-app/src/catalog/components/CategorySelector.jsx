...
export const parentIdOf = (category) =>
  category.parentId ?? category.parent_id ?? null;
...
export function categoryPath(categories, selectedId) {
  ...
  const path = [];
  let current = byId.get(Number(selectedId));
  const visited = new Set();
  while (current && !visited.has(current.id)) {
    path.unshift(current);
    visited.add(current.id);
    current = byId.get(Number(parentIdOf(current)));
  }
  return path;
}
...
export function categoryRows(categories) {
  ...
  categories.forEach((category) => {
    const parent = Number(parentIdOf(category)) || 0;
  ...
  const visit = (parent, depth) =>
    (children.get(parent) || [])
      ...
      .forEach((category) => {
        ...
        visit(Number(category.id), depth + 1);
      });
  visit(0, 0);
  return rows;
}
...
export default function CategorySelector({
  ...
  let depth = 0;
  while (true) {
    const choices = usable.filter(
      (category) =>
        (Number(parentIdOf(category)) || null) === (Number(parent) || null),
    );
  ...
  const choose = (level, next) => {
    if (!next) onChange(level ? path[level - 1]?.id || "" : "");
    else onChange(next);
  };
  return (
    <Grid container spacing={1.25}>
      {levels.map((level, index) => (
        <Grid
          key={index}
          size={{ xs: 12, md: Math.max(3, 12 / Math.min(levels.length, 4)) }}
        >
          <TextField
            select
            fullWidth
            required={required && index === 0}
            label={index === 0 ? label : `Subcategory level ${index}`}
            value={level.selected}
            onChange={(event) => choose(index, event.target.value)}
          >
            <MenuItem value="">
              {index === 0 ? "Select category" : "Use category above"}
            </MenuItem>
            {level.choices.map((category) => (
...
function TreeNode({
  ...
  return (
    ...
    >
      <ButtonBase
        component="div"
        onClick={() => onChange(node.category.id)}
        sx={{
          width: "100%",
        ...
        {selected && (
          <Stack direction="row" spacing={0.75} alignItems="center">
            ...
            <Button
              ...
              onClick={(event) => {
                event.stopPropagation();
                onChange(parentIdOf(node.category));
              }}
            >
              Unselect
            </Button>
...
export function CategoryTreeSelector({
  ...
  const initialPath = categoryPath(usable, value)
    .slice(0, -1)
    .map((category) => Number(category.id));
  const [expanded, setExpanded] = useState(new Set(initialPath));
  const path = categoryPath(usable, value);
  const selectedRoot = forest.find(
    (node) => Number(node.category.id) === Number(path[0]?.id),
  );
  const choose = (id) => {
    onChange(id);
    const ancestors = categoryPath(usable, id)
      .slice(0, -1)
      .map((category) => Number(category.id));
    ...
    <Box>
      ...
      >
        <Box>
          ...
          <Typography variant="caption" color="text.secondary">
            Choose a top-level category, then browse its subcategory tree.
          </Typography>
        </Box>
        {path.length > 0 && (
          <Chip
            color="primary"
            variant="outlined"
            label={path.map((category) => category.name).join(" › ")}
          />
        )}
      </Stack>
      <TextField
      ...
      >
        <MenuItem value="">Select top-level category</MenuItem>
        {forest.map((node) => (
      ...
      {selectedRoot?.children.length > 0 && (
        <Paper
          variant="outlined"
          sx={{ p: 1, mt: 1.25, maxHeight: 310, overflowY: "auto" }}
        >
          <Typography
            variant="caption"
          ...
          >
            Select a subcategory below, or keep {selectedRoot.category.name} as
            the master category.
          </Typography>
          {selectedRoot.children.map((node) => (
            <TreeNode
