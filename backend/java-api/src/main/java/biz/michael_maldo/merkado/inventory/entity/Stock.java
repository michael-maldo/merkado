...
@Entity @Table(schema = "inventory", name = "stock")
@Getter @Setter @NoArgsConstructor
public class Stock {
    @Id @GeneratedValue(strategy = GenerationType.IDENTITY) private Long id;
    @ManyToOne(optional = false, fetch = FetchType.EAGER) @JoinColumn(name = "product_id") private Product product;
    @OneToOne(optional = false, fetch = FetchType.EAGER) @JoinColumn(name = "variant_id", unique = true) private ProductVariant variant;
