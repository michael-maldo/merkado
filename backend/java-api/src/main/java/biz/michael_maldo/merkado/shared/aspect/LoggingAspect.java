...
public class LoggingAspect {
  ...
  public void logAfterReturning(JoinPoint joinPoint, Object result) {
    ...
    if (isSensitiveClass(className)) {
      logger.info("[{}.{}] SUCCESS return=[PROTECTED]", className, methodName);
  ...
  private Object sanitizeArgs(Object[] args) {
    return Arrays.stream(args).map(this::sanitizeObject).toList();
  }
  ...
  private Object sanitizeObject(Object obj) {
     ...
     */
    if (
      value.matches("^[A-Za-z0-9-_]+=*\\.[A-Za-z0-9-_]+=*\\.[A-Za-z0-9-_+=/]*$")
    ) {
      return "[JWT_REDACTED]";
    }
    ...
    value = value.replaceAll("(?i)token=([^,\\]\\)]+)", "token=[REDACTED]");
