...
public class OrderService {
  ...
  public OrderDtos.View create(OrderDtos.Create request, String username) {
    ...
    for (var requested : request.items()) {
      var variant =
        requested.variantId() != null
          ? variants
              .findById(requested.variantId())
              .filter(v -> v.isActive() && v.getProduct().isActive())
              .orElseThrow(() ->
                new BusinessException("Variant not found or inactive")
              )
          : variants
              .findByProductIdAndDefaultVariantTrue(
              ...
              .filter(v -> v.isActive() && v.getProduct().isActive())
              .orElseThrow(() ->
                new BusinessException("Default variant not found or inactive")
              );
      if (!seen.add(variant.getId())) throw new BusinessException(
      ...
      var stock = stocks
        ...
        .orElseThrow(() ->
          new BusinessException("No stock record for " + variant.getSku())
        );
      if (
        stock.getAvailable() < requested.quantity()
      ...
      item.setSpu(product.getSpu());
      item.setSku(variant.getSku());
      item.setBarcode(variant.getBarcode());
      item.setProductName(product.getMasterName());
      item.setVariantName(variant.getVariantName());
      item.setVariantOptions(
        db
          .sql(
            "select coalesce(jsonb_object_agg(t.name,v.value) filter (where t.id is not null),'{}'::jsonb)::text from catalog.variant_option_assignments a join catalog.variant_option_types t on t.id=a.option_type_id join catalog.variant_option_values v on v.id=a.option_value_id where a.variant_id=:id"
          )
          .param("id", variant.getId())
          .query(String.class)
          .optional()
          .map(this::optionSnapshot)
    ...
    return OrderDtos.View.from(orders.save(order));
  }
  ...
  private Map<String, String> optionSnapshot(String json) {
    try {
      ...
      throw new BusinessException("Could not snapshot variant options");
    }
  }
  ...
  public List<OrderDtos.View> list() {
    return orders
      ...
      .stream()
      .map(OrderDtos.View::from)
      .toList();
  }

  @Transactional(readOnly = true)
  public List<OrderDtos.View> list(OrderStatus status) {
    return orders
  ...
  @Transactional(readOnly = true)
  public OrderDtos.View get(Long id) {
    return OrderDtos.View.from(order(id));
  }
  ...
  @Transactional
  public OrderDtos.View transition(
    Long id,
    OrderStatus target,
    String changedBy,
    String note
    ...
    boolean allowed =
      (current == OrderStatus.PAYMENT_PENDING &&
        (target == OrderStatus.PAYMENT_VERIFIED ||
          target == OrderStatus.CANCELLED)) ||
      (current == OrderStatus.PAYMENT_VERIFIED &&
        target == OrderStatus.PACKED) ||
    ...
    if (
      ...
      for (var item : order.getItems()) {
        var stock = stocks
          .findByVariantIdForUpdate(item.getVariant().getId())
          .orElseThrow();
        stock.setReserved(stock.getReserved() - item.getQuantity());
        if (target == OrderStatus.DISPATCHED) stock.setQuantity(
    ...
    db.sql(
      "insert into orders.order_history(order_id,from_status,to_status,changed_by,note) values(:o,:f,:t,:u,:n)"
    )
      .param("o", id)
      .param("f", current.name())
      .param("t", target.name())
      .param("u", changedBy)
      .param("n", note)
      .update();
    if (target == OrderStatus.PAYMENT_VERIFIED) db.sql(
      "insert into warehouse.shipments(order_id) values(:o) on conflict(order_id) do nothing"
    )
      .param("o", id)
      .update();
