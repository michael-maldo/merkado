...
public class AuditAspect {
    ...
    public void record(JoinPoint point) {
        try {
            ...
            String details=json.writeValueAsString(Map.of("handler",point.getSignature().toShortString(),"path",path));
            db.sql("insert into audit.audit_logs(username,action,resource_type,resource_id,details) values(:u,:a,:r,:i,cast(:d as jsonb))")
                    .param("u",username==null?"anonymous":username).param("a",request.getMethod()).param("r",resource).param("i",resourceId==null?"":resourceId).param("d",details).update();
